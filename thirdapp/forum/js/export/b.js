...
/*
     this.tplItem = function(data){

     var item = "<li class='aite-item' id='<%=id%>'><span class='aite-img avatar'><img src='<%=lastFeed.senderPhotoUrl%>' /></span>" +
     "<span class='name'><b><%=lastFeed.senderName%></b></span>" +
     "</li>";
...
/*
     ...
     var data;
     var criteria = $(".searchContent").val();
     var _this = this;
     $.ajax({
     type:'get',
     url:'/thirdapp/forum/recentContacts',
     ...
     data = resp;
     _this.tplJude(_this.aiteWrap,data);
     });
	...
	17: [function(require, module, exports) {
		...
		//var imgPreview = require('../component/imgPreview');
		var ImgUploader = require('../component/img-uploader');
		var wxUploadImage = require('../component/wxUploadImage');
		//var TopicList = require('../component/hot-topic-list').TopicList;
		...
		var sendMsgOpt = {
			...
			sendPosts: function sendPosts(obj, isOpen1, isOpen2, isBeyondCredit1, isBeyondCredit2, isAdd) {
				...
				if ($sendBtn.attr('data-section') == 'true' && !$topicCurr.length && !_this.blueprintId) {
					Tribe.util.tips('发帖前先选择一个分类版块吧', 2000);
					return false;
				}
		...
		var detailPageOpt = {
			init: function init() {
				...
				//买家秀
				$(".buyersShow-btn").on('click', function(e) {
					_this.getTheBuyersShow(e);
				});
			},
		...
		var buyersShow = {
			...
			showOwnCode: function showOwnCode() {

				var showWindow = function showWindow() {

						carePublic.isCarePublic(carePublicCb);
					};
				var carePublicCb = function carePublicCb(isBool) {
						if (!isBool && Tribe.util.is_weixin() && carePublic.publicArr.isAuthorize) {
							carePublic.showPublicWindow("关注公众号才能知道你买了什么啊");
						}
					};
				...
				carePublic.init(obj);
				if ($('#detail-msg-reply').length) {
					carePublic.isShare(showWindow, true);
				} else {
					carePublic.isShare(showWindow);
				}
			},
			editBuyersShowItem: function editBuyersShowItem() {
			...
			editOpenedBuyersShow: function editOpenedBuyersShow() {
				...
				$('body').on('click', '.j-openedBuyersShowItem', function() {
					...
					var buyersShow = {
						...
						goodName: $this.attr('data-goodName'),
						price: $this.attr('data-price'),
						picPath: $this.attr('data-picPath')
					};
					$('.j-buyersShow-wrapper').html('<div class="product-wrap uploadPic-shows j-buyersShowItem" data-url="' + buyersShow.url + '" data-goodName="' + buyersShow.goodName + '" data-price="' + buyersShow.price + '" data-picPath="' + buyersShow.picPath + '">\n                                              <a href="javascript:;" class="close-uploadPic j-closeBuyersShowItem">×</a>\n                                              <div class="product-img"><img src="' + buyersShow.picPath + '"/></div>\n                                              <div class="product-detail">\n                                                <h3 class="product-title ellipsis">' + buyersShow.goodName + '</h3>\n                                                <p class="product-price">' + buyersShow.price + '</p>\n                                                <p class="product-tips">商品信息以商家店铺显示为准</p>\n                                              </div>\n                                            </div>');
					$('.j-openedBuyersShow').hide();
				});
			},
			warningTpl: function warningTpl(text) {
				var tpl = '<div class="warmingWrap BoxWrap confirmStyle02">\n            <div class="confirm-bg"></div>\n            <div class="layer-box">\n                <a class="deleteBtn delBtn j-deleteWarningTpl">×</a>\n                <div class="a-y-s-d">\n                    <p class="a-y-s-d-content" style="text-align: center;">' + text + '</center></p>\n                </div>\n            </div>\n        </div>';
				return tpl;
			},
			delWarningTpl: function delWarningTpl() {
			...
			openBuyersShow: function openBuyersShow() {
				var that = this;
					...
					$.ajax({
						...
						if (resp && resp.message.subscribe) {
							$.ajax({
								...
								var openedBuyersShowTpl = [];
								if (resp.success) {
									var thirdGoodsList = resp.message.thirdGoodsList || [];
									if (thirdGoodsList.length) {
										for (var i = 0; i < thirdGoodsList.length; i++) {
											var item = thirdGoodsList[i];
											openedBuyersShowTpl.push('<div class="j-openedBuyersShowItem" data-url="' + item.detail_url + '" data-goodName="' + item.goodName + '" data-price="' + item.price + '" data-picPath="' + item.pic_thumb_path + '"><a href="javascript:;" class="product-wrap border border-b">\n                                                                  <div class="product-img"><img src="' + item.pic_thumb_path + '"/></div>\n                                                                  <div class="product-detail">\n                                                                    <h3 class="product-title ellipsis">' + item.goodName + '</h3>\n                                                                    <p class="product-price">￥' + item.price + '</p>\n                                                                    <p class="product-tips">商品信息以商家店铺显示为准</p>\n                                                                  </div></a></div>');
										}
									} else {
										openedBuyersShowTpl.push('<div class="yz-shop-uploadPic">你还没有在商户的有赞店铺买过东西</div>');
									}
									$('.j-openedBuyersShowContent').html(openedBuyersShowTpl.join('')).show();
									$('.j-openedBuyersShow').show();
								} else {
									if (resp.message.error_code === 35011) {
										$('body').append(that.warningTpl('获取不到你的购买记录,因为管理员还没把公众号授权给有赞店铺,快去提醒管理员吧!'));
									} else if (resp.message.error_code === 33002) {
										openedBuyersShowTpl.push('<div class="yz-shop-uploadPic">你还没有在商户的有赞店铺买过东西</div>');
										$('.j-openedBuyersShowContent').html(openedBuyersShowTpl.join('')).show();
										$('.j-openedBuyersShow').show();
									} else if (resp.message.error_description === 'weixin follower not found') {
										openedBuyersShowTpl.push('<div class="yz-shop-uploadPic">你还没有在商户的有赞店铺买过东西</div>');
										$('.j-openedBuyersShowContent').html(openedBuyersShowTpl.join('')).show();
										$('.j-openedBuyersShow').show();
									} else {
										$('body').append(that.warningTpl('调用有赞接口错误!'));
									}
								}
							});
						} else {
		...
		var commonBuyersShow = {
			$buyersShowPage: $('.j-buyersShowPage'),
			init: function init() {
				var isOpenBuyersShow = $('#editor').attr('data-isOpenBuyersShow') === 'true';
				if ($('#editor').attr('data-isBuyersShow') === 'true') {
					if (isOpenBuyersShow) {
						buyersShow.delWarningTpl();
						this.buyersShowPageInit(true);
						this.$buyersShowPage.fadeIn();
						this.chooseBuyersShow();
					} else {
						buyersShow.delWarningTpl();
						this.buyersShowPageInit(false);
						this.$buyersShowPage.fadeIn();
						this.chooseBuyersShow();
					}
				}
			},
			buyersShowPageInit: function buyersShowPageInit(isOpenBuyersShow) {
				var that = this;
				var height = $(window).height();
				this.$buyersShowPage.css('height', height + 'px');
				var openedBuyersShowTpl = [];
				if (isOpenBuyersShow) {
					$.ajax({
						url: '/thirdapp/forum/isFocusPublic',
							...
							$.ajax({
								...
								if (resp.success) {
									...
									if (thirdGoodsList.length) {
										...
										openedBuyersShowTpl.push('<div class="commonBuyersShow-tips">你还没有在商户的有赞店铺买过东西</div>');
									}
									that.$buyersShowPage.html(openedBuyersShowTpl.join(''));
								} else {
									if (resp.message.error_code === 35011) {
										openedBuyersShowTpl.push('<div class="commonBuyersShow-tips">获取不到你的购买记录,因为管理员还没把公众号授权给有赞店铺,快去提醒管理员吧!</div>');
										that.$buyersShowPage.html(openedBuyersShowTpl.join(''));
									} else if (resp.message.error_code === 33002) {
										openedBuyersShowTpl.push('<div class="commonBuyersShow-tips">你还没有在商户的有赞店铺买过东西</div>');
										that.$buyersShowPage.html(openedBuyersShowTpl.join(''));
										...
										openedBuyersShowTpl.push('<div class="commonBuyersShow-tips">调用有赞接口错误!</div>');
										that.$buyersShowPage.html(openedBuyersShowTpl.join(''));
									}
								}
							});
						} else {
							buyersShow.showOwnCode();
						}
					});
				} else {
					openedBuyersShowTpl.push('<div class="commonBuyersShow-tips">你的社区还没开通买家秀功能!</div>');
					that.$buyersShowPage.html(openedBuyersShowTpl.join(''));
				}
			},
			chooseBuyersShow: function chooseBuyersShow() {
				var that = this;
				$('body').on('click', '.j-buyersShowPage .j-openedBuyersShowItem', function(e) {
					var $this = $(this);
					$this.addClass('j-buyersShowItem').siblings().removeClass('j-buyersShowItem');
					$('.j-buyersShowPageChosenItem').html($this.html());
					that.$buyersShowPage.fadeOut();
				});
				$('.j-buyersShowPageChosenItem').on('click', function() {
					var $this = $(this);
					$this.html('');
					that.$buyersShowPage.fadeIn();
				});
			},
			...
			scrollHidden: function scrollHidden() {
				var _height = $(window).height();
				$('body').css("height", _height + 'px');
				document.body.style.overflow = 'hidden';
				document.documentElement.style.overflow = 'hidden';
		...
		var freeCall = {
			...
			isOpenFreeCall: function isOpenFreeCall() {
				var me = this;
				new RequestUrl().getNetworkInfo().then(function(resp) {
					if (!resp) {
						me.callWrap.hide();
						return;
					}
					var isOpen = resp.networkVO.advancedOptions.openFreeCall;
					if (isOpen) {
						me.callWrap.show();
						...
						var option = {
							"continueCB": function continueCB() {
						...
						openCallFree.init(option);
						openCallFree.getUserInfo().then(function(resp) {
							if (!resp || !resp.success) {
								openCallFree.validMobile = false;
								return;
							}

							openCallFree.validMobile = resp.message.validMobile;
						});
					} else {
						me.callWrap.hide();
					}
			...
			addCall: function addCall() {
				if (openCallFree.validMobile) {
					...
					openCallFree.bindMobileTpl(option);
				}
			},
			callTpl: function callTpl() {
				if ($(".j-call-wrap").length != 0) {
					$(".j-call-wrap").show();
					return;
				}
				var str = "   <a class='j-call-wrap site-btn'>" + "       <i class='contact-icon'></i>" + "       <span class='ellipsis'>免费电话</span>" + "       <span class='site-close j-call-closeBtn'></span>" + "   </a>";
				$('.j-call-free').append(str).show();
			...
			closeCallIcon: function closeCallIcon() {
				$('.j-call-wrap').remove();
			}
		};
